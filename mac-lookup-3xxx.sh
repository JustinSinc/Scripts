...
# for use with Cisco Catalyst 3550 and 3750-series switches
...
while read -r i; do
        ...
        # store fourth column into port variable
...
echo -e "port,vlan,mac,vendor" > "$tempfile.sorted"

# sort the list by order of port
sort -V "$tempfile" >> "$tempfile.sorted"

# create the final output file
echo "" > "$tempfile.output"
cat "$tempfile.sorted" >> "$tempfile.output"
echo "" >> "$tempfile.output"

# list number of mac addresses per port on any port with multiple
for dup in $(cut -d "," -f1 "$tempfile.sorted" | uniq -d); do
        grep -wo -- "$dup" "$tempfile.sorted" | uniq -c | awk {'print $2 " has " $1 " mac addresses"'} >> "$tempfile.output"
done

# add one blank line to the end of the output file
echo "" >> "$tempfile.output"

# display location of tempfile for debugging purposes
echo "Output stored in "$tempfile.output""
